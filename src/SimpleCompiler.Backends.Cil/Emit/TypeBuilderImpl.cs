...
namespace SimpleCompiler.Backend.Cil.Emit
{
    internal sealed class TypeBuilderImpl : TypeBuilder
    {
        ...
        private void ValidateMethods()
        {
            for (int i = 0; i < _methodDefinitions.Count; i++)
            {
                ...
                if ((methodAttrs & MethodAttributes.Abstract) != 0)
                {
                    ...
                    {
                        throw new InvalidOperationException("TODO: Fill in with result of SR.InvalidOperation_BadTypeAttributesNotAbstract");
                    }

                    // If this is an abstract method or an interface should not have body.
                    if (body != null && body.ILOffset > 0)
                    {
                        throw new InvalidOperationException("TODO: Fill in with result of SR.Format(SR.InvalidOperation_BadMethodBody, method.Name)");
                    }
                }
                else if ((body == null || body.ILOffset == 0) && !method._canBeRuntimeImpl)
                {
                    throw new InvalidOperationException("TODO: Fill in with result of SR.Format(SR.InvalidOperation_BadEmptyMethodBody, method.Name)");
                }
            }
        }
        ...
        internal void ThrowIfCreated()
        {
            if (_isCreated)
            {
                throw new InvalidOperationException("TODO: Fill in with result of SR.InvalidOperation_TypeHasBeenCreated");
            }
        }
            ...
            Type[]? parameterTypes, Type[][]? requiredCustomModifiers, Type[][]? optionalCustomModifiers)
        {
            ...
            {
                throw new InvalidOperationException("TODO: Fill in with result of SR.InvalidOperation_ConstructorNotAllowedOnInterface");
            }
        ...
        private ConstructorBuilderImpl DefineDefaultConstructorInternal(MethodAttributes attributes)
        {
            ...
            if (con == null)
            {
                throw new NotSupportedException("TODO: Fill in with result of SR.NotSupported_NoParentDefaultConstructor");
            }
        ...
        protected override FieldBuilder DefineInitializedDataCore(string name, byte[] data, FieldAttributes attributes)
        {
            ...
            {
                throw new ArgumentException("TODO: Fill in with result of SR.Argument_BadSizeForData", nameof(data.Length));
            }
        ...
        protected override void DefineMethodOverrideCore(MethodInfo methodInfoBody, MethodInfo methodInfoDeclaration)
        {
            ...
            if (!ReferenceEquals(methodInfoBody.DeclaringType, this))
            {
                ...
                throw new ArgumentException("TODO: Fill in with result of SR.ArgumentException_BadMethodImplBody");
            }
            ...
            if (_methodOverrides.TryGetValue(baseType, out List<(MethodInfo ifaceMethod, MethodInfo targetMethod)>? im))
            {
                ...
                {
                    throw new ArgumentException("TODO: Fill in with result of SR.Format(SR.Argument_MethodOverriden, methodInfoBody.Name, FullName)", nameof(methodInfoDeclaration));
                }
        ...
        private ArgumentException ArgumentExceptionInvalidMethodOverride(string methodName) =>
            new("TODO: Fill in with result of SR.Format(SR.Argument_InvalidMethodOverride, FullName, methodName)", "methodInfoBody");
        ...
        protected override TypeBuilder DefineNestedTypeCore(string name, TypeAttributes attr,
            [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] Type? parent, Type[]? interfaces, PackingSize packSize, int typeSize) => _module.DefineNestedType(name, attr, parent, interfaces, packSize, typeSize, this);
            ...
            Type[][]? parameterTypeRequiredCustomModifiers, Type[][]? parameterTypeOptionalCustomModifiers, CallingConvention nativeCallConv, CharSet nativeCharSet)
        {
            ...
            {
                throw new ArgumentException("TODO: Fill in with result of SR.Argument_BadPInvokeMethod");
            }

            if ((_attributes & TypeAttributes.ClassSemanticsMask) == TypeAttributes.Interface)
            {
                throw new ArgumentException("TODO: Fill in with result of SR.Argument_BadPInvokeOnInterface");
            }
            ...
            {
                throw new ArgumentException("TODO: Fill in with result of SR.Argument_MethodRedefined");
            }
        ...
        protected override FieldBuilder DefineUninitializedDataCore(string name, int size, FieldAttributes attributes)
        {
            ...
            {
                throw new ArgumentException("TODO: Fill in with result of SR.Argument_BadSizeForData", nameof(size));
            }
...
#pragma warning restore SYSLIB0050
        ...
        internal void SetCustomAttribute(ConstructorInfo con, ReadOnlySpan<byte> binaryAttribute) => SetCustomAttributeCore(con, binaryAttribute);
            ...
            for (int i = 0; i < attributeInfo._namedParamNames.Length; ++i)
            {
                ...
                switch (name)
                {
                    ...
                    case "Size":
                        ...
                        break;
                    default:
                        throw new ArgumentException("TODO: Fill in with result of SR.Format(SR.Argument_UnknownNamedType, con.DeclaringType, name)", nameof(binaryAttribute));
                }
            }
        }
        ...
        protected override void SetParentCore([DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] Type? parent)
        {
            ...
            if (parent != null)
            {
                ...
                {
                    throw new ArgumentException("TODO: Fill in with result of SR.Argument_CannotSetParentToInterface");
                }
            ...
            else
            {
                ...
                else
                {
                    if ((_attributes & TypeAttributes.Abstract) == 0)
                    {
                        throw new InvalidOperationException("TODO: Fill in with result of SR.InvalidOperation_BadInterfaceNotAbstract");
                    }
        ...
        public override Type GetElementType() => throw new NotSupportedException("TODO: Fill in with result of SR.NotSupported_DynamicModule");
        ...
        public override Type UnderlyingSystemType
        {
            get
            {
                if (IsEnum)
                {
                    ...
                    {
                        throw new InvalidOperationException("TODO: Fill in with result of SR.InvalidOperation_NoUnderlyingTypeOnEnum");
                    }
        ...
        protected override bool IsCOMObjectImpl() => ((GetAttributeFlagsImpl() & TypeAttributes.Import) != 0) ? true : false;

        internal void ThrowIfNotCreated()
        {
            if (!_isCreated)
            {
                throw new NotSupportedException("TODO: Fill in with result of SR.NotSupported_TypeNotYetCreated");
            }
        }
                ...
                CallingConventions callConvention, Type[]? types, ParameterModifier[]? modifiers)
        {
            ...
            foreach (MethodBuilderImpl method in _methodDefinitions)
            {
                if (name.Equals(method.Name) && MatchesTheFilter(method, GetBindingFlags(method), bindingAttr, callConvention, types))
                {
                    if (found != null)
                    {
                        throw new AmbiguousMatchException("TODO: Fill in with result of SR.Format(SR.AmbiguousMatch_MemberInfo, method.DeclaringType, method.Name)");
                    }
            ...
            foreach (FieldBuilderImpl fieldInfo in _fieldDefinitions)
            {
                ...
                if (name.Equals(fieldInfo.Name, compare) && (bindingFlags & fieldFlags) == fieldFlags)
                {
                    if (match != null && ReferenceEquals(fieldInfo.DeclaringType, match.DeclaringType))
                    {
                        ...
                        throw new AmbiguousMatchException("TODO: Fill in with result of SR.Format(SR.AmbiguousMatch_MemberInfo, fieldInfo.DeclaringType, fieldInfo.Name)");
                    }
        ...
        [DynamicallyAccessedMembers(Constants.GetAllMembers)]
        ...
        private void ValidateInterfaceType(Type interfaceType)
        {
            ...
            {
                throw new ArgumentException("TODO: Fill in with result of SR.Argument_MustBeInterface", nameof(interfaceType));
            }

            if (!IsInterfaceImplemented(interfaceType))
            {
                throw new ArgumentException("TODO: Fill in with result of SR.Argument_InterfaceNotFound", nameof(interfaceType));
            }

            if (IsGenericParameter)
            {
                throw new InvalidOperationException("TODO: Fill in with result of SR.Argument_GenericParameter");
            }
        }
