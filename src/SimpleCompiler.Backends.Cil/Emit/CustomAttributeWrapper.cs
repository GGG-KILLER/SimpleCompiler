...
namespace SimpleCompiler.Backend.Cil.Emit
{
    ...
    internal struct CustomAttributeInfo
    {
        ...
        internal static CustomAttributeInfo DecodeCustomAttribute(ConstructorInfo ctor, ReadOnlySpan<byte> binaryAttribute)
        {
            ...
            {
                throw new ArgumentException("TODO: Fill in with result of SR.Format(SR.Argument_InvalidCustomAttributeLength, ctor.DeclaringType, binaryAttribute.Length)", nameof(binaryAttribute));
            }
            if ((binaryAttribute[0] != 0x01) || (binaryAttribute[1] != 0x00))
            {
                throw new ArgumentException("TODO: Fill in with result of SR.Format(SR.Argument_InvalidProlog, ctor.DeclaringType)", nameof(binaryAttribute));
            }
            ...
            for (int i = 0; i < numNamed; ++i)
            {
                ...
                {
                    throw new ArgumentException("TODO: Fill in with result of SR.Format(SR.Argument_UnknownNamedType, ctor.DeclaringType, namedType)", nameof(binaryAttribute));
                }
            }
        ...
        private static string StringFromBytes(ReadOnlySpan<byte> data, int pos, int len) => System.Text.Encoding.UTF8.GetString(data.Slice(pos, len));
        ...
        private static object? DecodeCustomAttributeValue(Type t, ReadOnlySpan<byte> data, int pos, out int rpos)
        {
            ...
            throw new NotImplementedException("TODO: Fill in with result of SR.Format(SR.NotImplemented_TypeForValue, t)");
        }
        ...
        private static Type ElementTypeToType(SerializationTypeCode elementType) =>
            elementType switch
            {
                ...
                _ => throw new ArgumentException("TODO: Fill in with result of SR.Argument_InvalidTypeCodeForTypeArgument", "binaryAttribute"),
            };
    }
}
